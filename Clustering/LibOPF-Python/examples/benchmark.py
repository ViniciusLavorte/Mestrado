...
def benchmark(data, target, n_samples):
	...
	for i, size in enumerate(list_n_samples):
		...
		def _bayes():
			...
			predicted = clf.predict(data_test)
			bayes_results[i,0] = precision_score(label_test, predicted)
			bayes_results[i,1] = recall_score(label_test, predicted)
			bayes_results[i,2] = f1_score(label_test, predicted)
			gc.collect()
		...
		def _linear():
			...
			predicted = clf.predict(data_test)
			linear_results[i,0] = precision_score(label_test, predicted)
			linear_results[i,1] = recall_score(label_test, predicted)
		...
		def _sgd():
			...
			linear_results[i,3] = time()-t
			predicted = clf.predict(data_test)
			sgd_results[i,0] = precision_score(label_test, predicted)
			sgd_results[i,1] = recall_score(label_test, predicted)
			sgd_results[i,2] = f1_score(label_test, predicted)
			gc.collect()
		...
		def _tree():
			...
			predicted = clf.predict(data_test)
			tree_results[i,0] = precision_score(label_test, predicted)
			tree_results[i,1] = recall_score(label_test, predicted)
